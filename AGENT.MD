...
- 架构：将大模型相关逻辑置于 `src/modules/agent/` 下，游戏机制相关逻辑置于 `src/modules/game/` 下，入口为 `src/main_single.js`。
...
## 调试与扩展
- 调试：`DEBUG=1 node src/main_single.js`（固定剧本与人数）
- 扩展：
